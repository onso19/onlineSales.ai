...
//     // const [, setFileUploadEnabled] = useState(false);
...
//         const maxSize = 5 * 1024 * 1024; // Example max file size (5 MB)
//         if (file.size > maxSize) {
//             setFormErrors({ ...formErrors, [id]: `* File size exceeds the limit of 5MB.` });
//             return;
//         }

//         // If file passes validation, store it in form data
...
//         setShowSubmittedForms(!showSubmittedForms); // Toggle the state
//     };
...
//             newFields.push(field); // Re-add the removed field to the end
//             return newFields;
//         });
//     };
...
//                 // { type: 'checkbox', placeholder: 'Agree to Terms' },
...
//                         {formErrors[field.id] && <span className='error'>{formErrors[field.id]}</span>}
//                         <button type="button" onClick={() => removeField(field.id, index)}>Remove</button>
//                     </div>
//                 ))}
//                 {removedFields.map((field) => (
//                     <div key={field.id} className='removed-button'>
